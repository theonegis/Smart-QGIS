...
class ChatWorker(QtCore.QObject):
    ...
    def _chat_loop(self, tools):
        while self.is_running:
            ...
            QgsMessageLog.logMessage(
                f"Sending request to Ollama with model: {self.model}",
                LOG_TAG,
                Qgis.Info,
            )
            QgsMessageLog.logMessage(
                f"Number of messages: {len(self.messages)}", LOG_TAG, Qgis.Info
            )
            QgsMessageLog.logMessage(
                f"Number of tools: {len(tools)}", LOG_TAG, Qgis.Info
            )
            ...
            if collected_tool_calls and self.is_running:
                QgsMessageLog.logMessage(
                    f"Tool calls requested: {len(collected_tool_calls)}",
                    LOG_TAG,
                    Qgis.Info,
                )
                for tool_call in collected_tool_calls:
                    ...
                    QgsMessageLog.logMessage(
                        f"Calling tool: {function_name} with {arguments}",
                        LOG_TAG,
                        Qgis.Info,
                    )
...
class ChatDockWidget(QtWidgets.QDockWidget):
    ...
    def handle_send(self):
        ...
        ai_html = (
            f'<div style="margin: 10px 0">' f"<strong>AI智能体: </strong><br/></div>"
        )
        ...
        self.worker.error.connect(
            lambda e: QgsMessageLog.logMessage(
                f"Chat error: {e}", LOG_TAG, Qgis.Critical
            )
        )
        self.thread.start()
